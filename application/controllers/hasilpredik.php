...
class hasilpredik extends CI_Controller
{
    public function index()
    {
        ...
        $this->load->view('admin/hasilpredik', $data);
    ...
    public function get_bulan_tahun($modelbarang, $ukuran)
    {

        //mengambil data dari database dengan modelbarang sesuai isi prameter
        // $this->db->group_by('ukuran');
        $this->db->select('YEAR(bulan) as tahun2');
        $this->db->group_by('tahun2');
        $tahun = $this->db->get_where('data_bulanan', ['modelbarang' => $modelbarang, 'ukuran' => $ukuran])->result();
        // print_r($tahun);

        $this->db->select('MONTH(bulan) as bulan2');
        $this->db->group_by('bulan2');
        $bulan = $this->db->get_where('data_bulanan', ['modelbarang' => $modelbarang, 'ukuran' => $ukuran])->result();
        // print_r($bulan);
        // die;
        //mengambil data ukuran
        $hbulan = '<option value="">Pilih Bulan</option>';
        foreach ($bulan as $dt) {
            // array_push($ukuran, $dt['ukuran']);
            $hbulan .= '<option value="' . $dt->bulan2 . '">' . bulan($dt->bulan2) . '</option>';
        }

        $htahun = '<option value="">Pilih Tahun</option>';
        foreach ($tahun as $dt) {
            // array_push($ukuran, $dt['ukuran']);
            $htahun .= '<option value="' . $dt->tahun2 . '">' . ($dt->tahun2) . '</option>';
        }
        // return data json agar bisa di baca oleh javascript
        // echo json_encode(array_unique($ukuran));
        // echo $hasil;
        echo json_encode(array('bulan' => $hbulan, 'tahun' => $htahun));
    }
